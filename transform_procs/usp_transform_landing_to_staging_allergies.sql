-- Transform Landing_Allergies to Staging_Allergies (chuẩn hóa, làm sạch)


CREATE OR ALTER PROCEDURE dbo.usp_transform_landing_to_staging_allergies
AS
BEGIN
    ...
    BEGIN TRY
        SELECT
            ...
            update_at
        INTO #src
        FROM DW_Synthea_Landing.dbo.Landing_Allergies
        WHERE PATIENT IS NOT NULL AND LTRIM(RTRIM(PATIENT)) <> '';

        MERGE DW_Synthea_Staging.dbo.Staging_Allergies AS t
        USING #src AS s
