-- ============================================================================
-- Stored Procedures: Load from [DW_Synthea_Staging] -> [DW_Synthea_DWH] (v5)
-- ============================================================================
-- Version    : 5.0
--
-- CHANGES IN v5
...
BEGIN
    ...
    SELECT
        dbo.fn_date_key(d.full_date), d.full_date,
        DATEPART(YEAR, d.full_date), DATEPART(QUARTER, d.full_date), DATEPART(MONTH, d.full_date),
        DATENAME(MONTH, d.full_date), DATEPART(ISOWK, d.full_date), DATEPART(DAY, d.full_date),
...
BEGIN
    ...
    SELECT p.Id AS patient_id, p.FIRST AS first_name, p.LAST AS last_name, p.BIRTHDATE AS birthdate, p.DEATHDATE AS deathdate,
...
BEGIN
    ...
    ;WITH src AS (
        SELECT e.Id AS encounter_id, CONVERT(DATE, e.[START]) AS start_date, CONVERT(DATE, e.[STOP]) AS stop_date, e.PATIENT AS patient_id, 
               e.PROVIDER AS provider_id, e.PAYER AS payer_id, e.ORGANIZATION AS organization_id, e.ENCOUNTERCLASS AS encounter_class, 
               e.CODE AS encounter_code, e.REASONCODE AS reason_code, e.REASONDESCRIPTION AS reason_description, 
               ISNULL(e.BASE_ENCOUNTER_COST,0) AS base_encounter_cost, ISNULL(e.TOTAL_CLAIM_COST,0) AS total_claim_cost, 
    ...
    INSERT INTO dbo.fact_encounter (encounter_id, start_date_key, stop_date_key, patient_key, provider_key, payer_key, organization_key, encounter_class, encounter_code, reason_code, reason_description, base_encounter_cost, total_claim_cost, payer_coverage, is_hospitalized, length_of_stay_days, encounter_count)
    SELECT 
        ...
        CASE WHEN s.encounter_class IN ('inpatient') THEN 1 ELSE 0 END,
        CASE WHEN s.stop_date IS NOT NULL THEN DATEDIFF(DAY, s.start_date, s.stop_date) ELSE 0 END,
        1
    FROM src s
...
BEGIN
    ...
    SELECT 
        ...
        organization_key, payer_key, encounter_class,
        ...
        SUM(total_claim_cost - payer_coverage) AS total_out_of_pocket,
    ...
    WHERE start_date_key BETWEEN dbo.fn_date_key(@FromDate) AND dbo.fn_date_key(@ToDate)
      AND organization_key IS NOT NULL AND payer_key IS NOT NULL
    GROUP BY start_date_key, organization_key, payer_key, encounter_class;
END;
GO
...
BEGIN
    ...
    ;WITH src AS (
        SELECT c.[START] AS start_date, c.[STOP] AS stop_date, c.PATIENT AS patient_id, c.ENCOUNTER AS encounter_id, c.CODE AS condition_code, 
               e.PROVIDER AS provider_id, e.ORGANIZATION AS organization_id
...
BEGIN
    ...
    ;WITH daily_calc AS (
        SELECT 
            ...
            SUM(CASE WHEN e.is_hospitalized = 1 AND e.start_date_key <= d.date_key AND (e.stop_date_key IS NULL OR e.stop_date_key >= d.date_key) THEN 1 ELSE 0 END) AS hospitalized_cases
        FROM dbo.fact_conditions fc
        CROSS JOIN dbo.dim_date d
        LEFT JOIN dbo.fact_encounter e ON e.encounter_id = fc.encounter_id 
            AND e.start_date_key <= d.date_key 
            AND (e.stop_date_key IS NULL OR e.stop_date_key >= d.date_key)
    ...
    INSERT INTO dbo.fact_condition_daily (date_key, condition_code_key, organization_key, new_cases, active_cases, resolved_cases, cumulative_cases, hospitalized_cases)
...
BEGIN
    ...
    ;WITH src AS (
        SELECT m.PATIENT AS patient_id, CONVERT(DATE, m.[START]) AS start_date, ISNULL(m.CODE, 'UNKNOWN') AS medication_code,
               MAX(CONVERT(DATE, m.[STOP])) AS stop_date, MAX(m.PAYER) AS payer_id, MAX(m.ENCOUNTER) AS encounter_id,
               MAX(m.[DESCRIPTION]) AS medication_description, MAX(m.REASONCODE) AS reason_code,
               MAX(e.PROVIDER) AS provider_id, MAX(e.ORGANIZATION) AS organization_id,
               SUM(ISNULL(m.BASE_COST, 0)) AS base_cost, SUM(ISNULL(m.PAYER_COVERAGE, 0)) AS payer_coverage, 
    ...
    SELECT 
        p.patient_key, dbo.fn_date_key(s.start_date), s.medication_code, 
        CASE WHEN s.stop_date IS NULL OR s.stop_date < '1900-01-01' OR s.stop_date > '2099-12-31' THEN NULL ELSE dbo.fn_date_key(s.stop_date) END,
...
CREATE OR ALTER PROCEDURE dbo.usp_run_dwh_load
    @FromDate DATE, @ToDate DATE, @AsOfDate DATE = NULL
AS
BEGIN
    SET NOCOUNT ON;
    IF @AsOfDate IS NULL SET @AsOfDate = CONVERT(DATE, GETDATE());
